...
export const PREV_HOOK = "pre-commit.han.sip-prev";
/** Commit rilis yang berisi detector. Bukan tag — tag bisa digeser. */
export const PIN_SHA = "fa0b9ce08b794cc18f6fabbbcbc20973a3752911";
export const NPX = `github:ganezha/kotak-kecil#${PIN_SHA}`;

export function hookBody() {
  return `#!/bin/sh
# ${HOOK_MARK} — jalankan lagi kalau file ini hilang
# Pin: commit SHA 40 hex (PIN_SHA), bukan main / tag
root="$(git rev-parse --show-toplevel)" || exit 2
prev="$(dirname "$0")/${PREV_HOOK}"
if [ -f "$prev" ]; then
  if [ -x "$prev" ]; then
    "$prev" "$@" || exit $?
  else
...
# npx github: di npm 10 sering gagal GitFetcher. Fail closed.
echo "han.sip: tidak ada cli lokal atau node_modules/.bin/han.sip" >&2
echo "han.sip: pasang lewat: npm install ${NPX}" >&2
...
export async function pasang({ check = false } = {}) {
  ...
  if (bak) {
    const prev = path.join(path.dirname(dest), PREV_HOOK);
    await writeFile(prev, existing, { encoding: "utf8", mode: 0o755 });
    await chmod(prev, 0o755);
    console.warn(`han.sip pasang: hook asing. Chain: ${prev} → han.sip`);
    console.warn(`han.sip pasang: cadangan: ${bak}`);
  }
