...
test("symlink bin (npx) tetap menjalankan main", async () => {
  ...
  try {
    ...
    assert.match(stdout, new RegExp(String.raw`npm install github:ganezha/kotak-kecil#[0-9a-f]{40}`));
    assert.match(stdout, /npx han\.sip \./);
...
test("han.sip pasang: hook self-contained, --check sip", async () => {
  ...
  try {
    ...
    assert.match(hook, /node_modules\/\.bin\/han\.sip/);
...
test("npx pin: pasang + template + salinan = commit SHA", async () => {
  ...
  assert.equal(hookBody().includes("node_modules/.bin/han.sip"), true);
